...
func getEntityId(record truthset.Record) int64 {
	...
	response, err := g2engine.GetEntityByRecordID(ctx, record.DataSource, record.Id)
	if err != nil {
		return result
	}
...
func setupSenzingConfig(ctx context.Context, moduleName string, iniParams string, verboseLogging int) error {
	...
	for _, truthsetSource := range truthset.TruthsetDataSources {
		_, err := aG2config.AddDataSource(ctx, configHandle, truthsetSource.Data)
		if err != nil {
...
func ExampleG2engineImpl_FindNetworkByEntityID() {
	...
	fmt.Println(truncate(result, 228))
...
func ExampleG2engineImpl_WhyRecords() {
	...
	fmt.Println(truncate(result, 120))
...
func ExampleG2engineImpl_ProcessWithInfo() {
	...
	fmt.Println(result)
	// Output: {"DATA_SOURCE":"CUSTOMERS","RECORD_ID":"555","AFFECTED_ENTITIES":[{"ENTITY_ID":200001}],"INTERESTING_ENTITIES":{"ENTITIES":[]}}
}
...
func ExampleG2engineImpl_ReevaluateRecordWithInfo() {
	...
	fmt.Println(result)
	// Output: {"DATA_SOURCE":"CUSTOMERS","RECORD_ID":"1001","AFFECTED_ENTITIES":[{"ENTITY_ID":200001}],"INTERESTING_ENTITIES":{"ENTITIES":[]}}
}
